...
func (m tuiModel) coverageBanner() string {
	...
	if m.coverageTagSweep {
		msg = fmt.Sprintf("⚠ A resource of any of these %d service(s) may not appear on this page if the resource does not have a tag!! Press c to view the resources.",
			m.coverageMissing)
	} else {
		msg = fmt.Sprintf("⚠ --typed-only: A resource of any of these %d service(s) may not appear on this page if the resource does not have a tag!! Press c to view the resources.",
			m.coverageMissing)
	}
	w := m.width - 2
	if w < 10 {
...
func (m *tuiModel) syncDetailViewport() {
	...
	// Subtract the border/padding (2) and two columns for the scrollbar gutter
	// (a spacer plus the bar, drawn in renderDetailPanel). The gutter is
	// reserved unconditionally so content does not reflow the moment the panel
	// becomes scrollable.
	vpWidth := m.detailWidth() - 2 - 2
	if vpWidth < 10 {
		vpWidth = 10
	}
