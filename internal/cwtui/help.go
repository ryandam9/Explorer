...
func helpSections() []helpSection {
	...
	return []helpSection{
		...
		{"[3] Log events", []row{
			...
			{"t", "Toggle the zebra-striped table view — Time and Message, with long messages wrapped; v shows the stream and every JSON field"},
			{"J", "Table view: expand JSON embedded in each message into indented lines inside the Message column"},
		...
		{"Log viewer (full-screen, Enter on an event)", []row{
			...
			{"t", "Table view of the streamed events — same Time/Message table as the events panel, with record view (v) and per-row copy (y)"},
			{"J", "Pretty-print JSON embedded in messages — in the line view and in the table's Message column alike (one preference, shared)"},
			{"/", "Find in the log"},
